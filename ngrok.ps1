Start-Sleep -Seconds 10  # Дай интернету время встать
...
# Получим путь текущего скрипта
$thisScript = $MyInvocation.MyCommand.Path
Log "Текущий скрипт: $thisScript"

# Если ещё не в автозагрузке — скопируем и выйдем
$startupPath = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\ngrok.ps1"
if ($thisScript -ne $startupPath -and !(Test-Path $startupPath)) {
    Copy-Item -Path $thisScript -Destination $startupPath -Force
    Log "Скрипт скопирован в автозагрузку"
    exit
}

# Включаем RDP и открываем порт 3389
try {
...
try {
    netsh advfirewall firewall add rule name="ngrok RDP" dir=in action=allow protocol=TCP localport=3389 | Out-Null
    Log "Firewall правило добавлено"
} catch {
    Log "Ошибка при добавлении firewall правила: $_"
}

# Скачиваем и распаковываем ngrok
